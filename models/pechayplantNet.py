...
SAVE_PATH = './out/pechayPlant/'

class pechayplantNet(object):

    def __init__(self, name='pechayplantNet', conv_base='resNet50', shape=(224,224,3)):
        self.name = name
    ...
    def build(self, loss='categorical_crossentropy', metrics='accuracy', lr=2e-5, momentum=0.9):
        ...
        # Base model = Pre-trained model using InceptionV3, ResNet, etc. architectures trained on imagenet dataset
        network = cnn_model(self.conv_base)

        # Specify input image
        leaf_img = keras.layers.Input(shape=self.shape, name=self.name)

        # Your specific model put on top of the base model
        # Network = base model & (leaf image) = input
        x = network(leaf_img)
        x = keras.layers.GlobalAveragePooling2D(name="avg_pool")(x)
        x = keras.layers.BatchNormalization()(x)
        x = keras.layers.Flatten()(x)

        # Custom Fully Connected layer = New classifier
        x = keras.layers.Dense(1024, activation='relu')(x)
        x = keras.layers.Dropout(0.5)(x)
        leaf_plant_disease = keras.layers.Dense(5, activation="softmax", name="disease")(x)

        x = keras.layers.Dense(1024, activation='relu')(x)
        x = keras.layers.Dropout(0.5)(x)
        leaf_severity = keras.layers.Dense(4, activation="softmax", name="severity")(x)

        self.model = keras.models.Model(leaf_img, [leaf_plant_disease,leaf_severity])
    ...
    def test(self,test_generator, steps, batch_size = 1,verbose=1):
         ...
         #applies the getGenerator function to the test_generator
         y_pred = self.model.predict(test_generator.getGenerator(),
    ...
    def evaluate(self,val_generator, steps, verbose=1):

         y_eval = self.model.evaluate(val_generator.getGenerator(),steps=steps,verbose=verbose)
